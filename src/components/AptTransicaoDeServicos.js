...
function AptTransicaoDeServicos({ transition }) {
  moment.locale('pt-br');
  ...
  const [viewtransition, setviewtransition] = useState(transition);
  ...
  if (viewtransition == 1) {
    return (
      <div className="menucover fade-in" style={{ zIndex: 9, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
        <div className="menucontainer" style={{width: '100vw', height: '100vh'}}>
          <div id="cabeçalho" className="cabecalho">
            <div className="title5">{'TRANSIÇÃO DE SERVIÇOS'}</div>
            <div id="botões" style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center' }}>
              <button className="red-button" onClick={() => setviewtransition(0)}>
                <img
                  alt=""
                  src={deletar}
                  style={{
...
export default AptTransicaoDeServicos;